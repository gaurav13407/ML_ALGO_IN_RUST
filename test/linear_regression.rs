...
use ML_ALGO_Rewite::{data, linear, model_io, preprocess, split};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (x, y): (Array2<f64>, Array1<f64>) =
        data::load_excel_by_name(path, target_column_name, None)?;
    ...
    use ML_ALGO_Rewite::{data, encoding, linear, metrics, model_io, preprocess, split};
    ...
    println!(
        "Compare: linear::mse = {:.6}, manual_mse = {:.6}",
        mse, mse_manual
    );
    ...
    let (loaded_coef, loaded_mean, loaded_std) =
        model_io::load_model("models/linear_model_rust.json")?;
    println!(
        "Loaded coef len: {}, mean len: {}, std len: {}",
        loaded_coef.len(),
        loaded_mean.len(),
        loaded_std.len()
    );

    let loaded_scaler = preprocess::StandardScaler {
        mean: loaded_mean.clone(),
        std: loaded_std.clone(),
    };
    ...
    let max_abs_diff = y_pred
        .iter()
        .zip(y_pred2.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0_f64, f64::max);
    println!(
        "Max abs diff between before-save and after-load predictions: {:.12}",
        max_abs_diff
    );
